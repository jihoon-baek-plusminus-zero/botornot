...
async function testProjectPrompt() {
  try {
    console.log('🔍 프로젝트 프롬프트 테스트 시작...');
    
    // 프로젝트에서 실제 사용하는 프롬프트 구조
    const systemPrompt = '당신은 AI vs Human 게임에서 AI 플레이어 역할을 합니다. 자연스럽고 인간다운 응답을 생성해주세요.'
    
    const userPrompt = `현재 게임 상황:
- 주제: 오늘 날씨가 정말 좋네요!
- 게임 타입: 1v1
- 대화 히스토리:
A: 안녕하세요! 오늘 날씨가 정말 좋네요. 뭐 하고 계세요?

당신은 Player A입니다. 위의 대화 히스토리를 참고하여 자연스럽고 인간다운 응답을 생성해주세요.
...
응답은 자연스럽고 casual한 톤으로 해주세요. 100자 이내로 답변해주세요.`;
    ...
    console.error('❌ 프로젝트 프롬프트 테스트 실패:');
