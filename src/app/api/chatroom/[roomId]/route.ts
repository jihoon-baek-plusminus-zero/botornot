  ...
  try {
    ...
    switch (action) {
      ...
      case 'process_ai_turn':
        return await handleProcessAITurn(roomManager, roomId)
...
async function handleProcessAITurn(
  roomManager: ChatRoomManager,
  roomId: string
) {
  try {
